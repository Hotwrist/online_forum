...
{
	cout << HTTPRedirectHeader("../admin/?username="+username);
}

void redirect_to_ordinary_users_page(string username)
{
	cout << HTTPRedirectHeader("../ordinary_users/?username="+username);
}
...
*/
bool save_sessionID(string sessionID, string username,string life_time, string IP) 
{
    try 
    {
		Driver *driver;
		...
		prep_stmt = con->prepareStatement("INSERT INTO Sessions (SessionID,fk_username,expires,IP,online) VALUES(?,?,?,?,?)");
...
/* The get_sessionID Function
    It creates a SessionID for the user and calls save_sessionID() to save it.
	...
	use of the C++ standard library <chrono>
...
string get_sessionID(string username) 
{
    ...
    if (!save_sessionID(sessionID,username, buf,ENV))
    {
        error_redirect("ERROR: An error occured. Please, login again");
    }
    return sessionID;
}
...
*/
void send_email(string username, string email, string code) 
{
	try 
    {
        Driver *driver;
		...
		if(username == "admin")
		{
			prep_stmt = con->prepareStatement("INSERT INTO admin_email(email,name,code) VALUES(?,?,?)");
...
/* The create_2fa Function
	This creates an OTP code, saves it to the database(2FA) and calls send_email() to send it to the specific user.
...
bool create_2fa(string username) 
{
    ...
    try 
    {
        ...
        prep_stmt = con->prepareStatement("INSERT INTO 2FA(code,fk_username,expires) VALUES(?,?,?)");
		...
		// Send the OTP Code to the email address specified.
...
/* The set_TOTP Function
    ...
    This creates a random seed by calling itob32() for the TOTP generation, and saves it to the database table :TOTP
    It uses the database user 'register'
...
string set_TOTP(string username) 
{
    ...
    try 
    {
        ...
        prep_stmt = con->prepareStatement("INSERT INTO TOTP(seed,fk_username,online) VALUES(?,?,?)");
...
void display_2fa()
{
	...
	{
        	cout << HTTPHTMLHeader() << endl;
        	cout << html() << head(title("Ordinary users 2-FA")).add(link().set("rel", "stylesheet").set("href", "../static_files/css/main.css")) << endl;
...
void display_3fa(string username= "")
{
	...
	{
        	cout << HTTPHTMLHeader() << endl;
       		cout << html() << head(title("Admin 3-FA")) << endl;
        	cout << link().set("rel", "stylesheet").set("href", "../static_files/css/qrcode.css") << endl;
        	cout << script().set("src", "../static_files/js/qrcode.js") << endl;
        	cout << script() << endl;
        	cout << head() << endl;
        	cout << body();
        	...
        	cout << script() << endl;
        	cout << p("Please, Scan the code above with a TOTP App like QR Scanner or Google Authenticator, to gain access.") << endl;
...
/*  The check_login Function
	...
	it also hashes the password by calling sha256() and compare it to the password hash
...
/* The main Function
	...
	It performs a login check. If successul, it creates a 2FA(for ordinary users) or 3FA(for admin),
...
{	
	...
	{
		error_redirect("ERROR: You did not supply a username.");
		exit(EXIT_SUCCESS);
	}
	...
	{
		// If the user is already online, check_login function will return false. This is because the 
		// function body uses the SELECT statement to retrieve values where online=0, and since online=1,
		// signifying the user is online, it will return false. Hence,  we have to use the negation operator,!, 
		// to make the if-block run, redirecting the specific user to his/her profile page.
		if((!(check_login(username, password))))
		{
			if(username == "admin") redirect_to_admin_page(username);
			else redirect_to_ordinary_users_page(username);
		}
	}
}
