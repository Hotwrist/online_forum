...
void error(string err_message)
{
	try
	{
		...
		cout << html() << head(title("Oxford Online Forum")) << endl;
        	cout << head() << endl;
        	cout << body();
...
/* The error_redirect Function
	...
	It pops the error message on the screen and redirects the user to the start/login page.
*/
...
{
	// Since the login page is one step up the directory, we use '..'
	cout << HTTPRedirectHeader("..");
}

/* The make_query() Function
	This function takes in 2 parameters -- query and username.
	
	->query: contains the sql command
	->username: contains the username(admin)
	
	This function performs an SQL SELECT operation to retrieve data as specified in the 'query'].
...
ResultSet *make_query(string query, string username) 
{
    try 
    {
		...
		// the user 'login' which is allowed SELECT access to the 'login' table of the database and
		// lastly, its password.
...
/* The admin_profile(string) Function
	It takes one parameter: username of type 'string'. 
	With the username, it makes request to the database for the admin
	username, full name, and email. On successful retrieval of these information,
	it displays it to the screen in a nice-formatted pattern using CSS.
*/
void admin_profile(string username)
{
	// Retrieve all information pertaining to the username(admin) from the 'login' table of the
	// database -- Users
	...
	try 
	{
        	...
        	cout << h1("Admin Profile Page").set("id", "profile-page");
...
/* The main Function
	...
	It then calls the admin_profile function with the argument of the username received.
	The admin_profile function displays the admin details.
*/
int main()
{	
	Cgicc form_data;
	...
	form_iterator fi = form_data.getElement("username");  
   	...
   	{  
      		...
      		// If the user has not been logged out, display his profile
      		if(is_online(username))
				admin_profile(username);
			else // user has been logged out, take him/her to the login page to login again!.
				redirect_to_login();
   	}
}
